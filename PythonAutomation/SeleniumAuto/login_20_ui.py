...
def readCsv(row, col):
    rows = []
    with open('FilesOfUI/UserList.csv') as f:  ## open the file
        reader = csv.reader(f)
        next(reader, None)
        for iter in reader:
            rows.append(iter)
...
class TestLoginOrion(unittest.TestCase):
    def readCsv(row, col):
        rows = []
        with open('FilesOfAPI/UserList.csv') as f:  ## open the file
            reader = csv.reader(f)
            next(reader, None)
            for iter in reader:
                rows.append(iter)
    ...
    def setUpClass(cls):  ## cls:for class,
        # test data
        # cls.driver = webdriver.Chrome('FilesOfUII/chromedriver')  # Ubuntu
        cls.driver = webdriver.Chrome('FilesOfUII/chromedriver.exe')  # Windows

        # self.driver.get = self.driver.get("https://orioncloud.io/dashboard")
        cls.driver.get = cls.driver.get("https://nbai.io/dashboard/#/login")
